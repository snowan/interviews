package leetcode.hashtable.LC1165;
...
public class LC1165SingleRowKeyboard {
  public int calculateTime(String keyboard, String word) {
    ...
    for (int i = 0; i < len; i++) {
      keyIndex.put(keyboard.charAt(i), i);
    }
    int steps = keyIndex.get(word.charAt(0));
    len = word.length();
    for (int i = 1; i < len; i++) {
      steps += Math.abs(keyIndex.get(word.charAt(i - 1)) - keyIndex.get(word.charAt(i)));
    }
    return steps;
  }
}
