package main.leetcode.dpgreedy.LC471;
      ...
      for (int i = 0; i < len - l + 1; i++) {
        ...
        if (l < 5) {
          ...
          for (int k = 0; k < currLen; k++) {
            if (currLen % (k + 1) != 0) continue;
            String firstSub = curr.substring(0, k + 1);
            if (curr.replaceAll(firstSub, "").equals("")) {
              String newStr = (currLen / (k + 1)) + "[" + dp[i][i + k] + "]";
