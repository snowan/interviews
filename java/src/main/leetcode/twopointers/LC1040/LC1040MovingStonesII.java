package main.leetcode.twopointers.LC1040;
...
public class LC1040MovingStonesII {
  ...
  public int[] numMovesStonesII(int[] stones) {
    ...
    Arrays.sort(stones);
    int max = Math.max(stones[len - 1] - stones[1], stones[len - 2] - stones[0]) - (len - 2);
    int min = len;
    int idx = 0;
    for (int i = 0; i < len; i++) {
      while (stones[i] - stones[idx] >= len) idx++;
      if (i - idx + 1 == len - 1 && stones[i] - stones[idx] == len - 2) {
        min = Math.min(min, 2);
      } else {
        min = Math.min(min, len - (i - idx + 1));
      }
    }
