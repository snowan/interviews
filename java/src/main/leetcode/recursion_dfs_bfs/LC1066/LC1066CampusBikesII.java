package main.leetcode.recursion_dfs_bfs.LC1066;
...
public class LC1066CampusBikesII {
  ...
  public int assignBikes(int[][] workers, int[][] bikes) {
    helper(workers, 0, bikes, 0, new boolean[bikes.length]);
    return min;
  }
  private void helper(int[][] workers, int start, int[][] bikes, int currSum, boolean[] visited) {
    ...
    for (int i = 0; i < bikes.length; i++) {
      ...
      visited[i] = true;
      helper(workers, start + 1, bikes, currSum + getDist(workers[start], bikes[i]), visited);
      visited[i] = false;
    }
  }
